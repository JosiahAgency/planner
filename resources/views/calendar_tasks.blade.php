        ...
        <div class="flex flex-col md:flex-row max-md:gap-3 items-center justify-between mb-5">
            <div class="flex items-center gap-4">
                ...
                <h6 class="text-xl leading-8 font-semibold text-gray-900">
                    {{ $carbonDate->isToday() ? 'Today, ' : '' }}{{ $carbonDate->format('F j Y') }}
                </h6>
            </div>
            <form method="GET" action="{{ route('home') }}"
                class="flex items-center gap-px rounded-lg bg-gray-100 p-1">
                <input type="hidden" name="date" value="{{ $carbonDate->toDateString() }}">
                <button name="view" value="day"
                    class="rounded-lg py-2.5 px-5 text-sm font-medium {{ $view === 'day' ? 'text-indigo-600 bg-white' : 'text-gray-500' }} transition-all duration-300 hover:bg-white hover:text-indigo-600">Day</button>
                <button name="view" value="week"
                    class="rounded-lg py-2.5 px-5 text-sm font-medium {{ $view === 'week' ? 'text-indigo-600 bg-white' : 'text-gray-500' }} transition-all duration-300 hover:bg-white hover:text-indigo-600">Week</button>
                <button name="view" value="month"
                    class="rounded-lg py-2.5 px-5 text-sm font-medium {{ $view === 'month' ? 'text-indigo-600 bg-white' : 'text-gray-500' }} transition-all duration-300 hover:bg-white hover:text-indigo-600">Month</button>
            </form>
        ...
        <div class="relative">
            @if ($view === 'month')
                <div class="grid grid-cols-7 border-t border-gray-200 sticky top-0 left-0 w-full">
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Mon</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Tue</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Wed</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-600">Thur</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Fri</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Sat</div>
                    <div class="p-3.5 flex items-center justify-center text-sm font-medium  text-gray-900">Sun</div>
                </div>
                <div class="grid grid-cols-7 w-full overflow-x-auto">
                    @foreach ($dates as $date)
                        <div
                            ...
                            {{ $date->format('j') }}
                            @if (isset($groupedTasks[$date->toDateString()]))
                                @foreach ($groupedTasks[$date->toDateString()] as $task)
                                    <div class="rounded p-1.5 border-l-2 border-indigo-600 bg-indigo-50 mb-2">
                                        <p class="text-xs font-normal text-gray-900 mb-px">{{ $task->title }}</p>
                                        @if ($task->start_time && $task->end_time)
                                            <p class="text-xs font-semibold text-indigo-600">{{ $task->start_time }} -
                                                {{ $task->end_time }}</p>
                                        @endif
                                    </div>
                                @endforeach
            ...
            @elseif ($view === 'week')
                <div class="grid grid-cols-7 border-t border-gray-200 sticky top-0 left-0 w-full">
                    @foreach ($dates as $date)
                        <div
                            class="p-3.5 flex items-center justify-center text-sm font-medium {{ $date->isToday() ? 'text-indigo-600' : 'text-gray-900' }}">
                            {{ $date->format('M j') }}
                        </div>
                    @endforeach
                </div>
            ...
            @elseif($view === 'day')
                <div class="p-4 border rounded bg-white">
                    <strong>{{ $carbonDate->format('l, F j, Y') }}</strong>
                    @if (isset($groupedTasks[$carbonDate->toDateString()]))
                        <ul class="mt-2 text-xs text-gray-700">
                            @foreach ($groupedTasks[$carbonDate->toDateString()] as $task)
                                <li class="mb-2">
                                    <div class="rounded p-1.5 border-l-2 border-indigo-600 bg-indigo-50">
                                        <span class="text-gray-900">{{ $task->title }}</span>
                                        @if ($task->start_time && $task->end_time)
                                            <span
                                                class="block text-xs font-semibold text-indigo-600">{{ $task->start_time }}
                                                - {{ $task->end_time }}</span>
                                        @endif
                                    </div>
                                </li>
                    ...
                    @else
                        <div class="text-gray-400">No tasks</div>
                    @endif
                </div>
            @endif
        </div>
